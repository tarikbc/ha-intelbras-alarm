...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if not errors:
            try:
                _LOGGER.info("Testing connection to panel at %s:%s", 
                            user_input[CONF_PANEL_IP], user_input[CONF_PORT])
    ...
    async def _test_connection(self, config: dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Testing basic connection to %s:%s", 
                         config[CONF_PANEL_IP], config.get(CONF_PORT, DEFAULT_PORT))
